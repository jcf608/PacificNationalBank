-- ============================================================
-- DDL for BRANCH_HIST_16_0091
-- Pacific National Bank — DB2 Schema
-- ============================================================
CREATE TABLE PNB.BRANCH_HIST_16_0091 (
    COL_000    VARCHAR(60) ,
    COL_001    DECIMAL(15,2) ,
    COL_002    VARCHAR(200) NOT NULL,
    COL_003    DECIMAL(9,6) ,
    COL_004    SMALLINT NOT NULL,
    COL_005    SMALLINT NOT NULL,
    COL_006    INTEGER ,
    COL_007    VARCHAR(200) ,
    COL_008    DECIMAL(15,2) NOT NULL,
    COL_009    INTEGER NOT NULL,
    COL_010    INTEGER NOT NULL,
    COL_011    DECIMAL(9,6) ,
    COL_012    CHAR(3) NOT NULL,
    COL_013    DATE NOT NULL,
    COL_014    TIMESTAMP ,
    COL_015    DECIMAL(9,6) NOT NULL,
    COL_016    DECIMAL(9,6) ,
    COL_017    CHAR(3) ,
    COL_018    INTEGER ,
    COL_019    CHAR(1) ,
    COL_020    BIGINT NOT NULL,
    COL_021    DECIMAL(15,2) ,
    COL_022    BIGINT ,
    COL_023    INTEGER ,
) IN PNBTS01;

CREATE UNIQUE INDEX PNB.IX_BRANCH_H160091_PK
    ON PNB.BRANCH_HIST_16_0091 (COL_000);

CREATE OR REPLACE PROCEDURE PNB.SP_BRANCH_H160091 (
    IN p_key CHAR(16),
    OUT p_result INTEGER
)
LANGUAGE SQL
BEGIN
    DECLARE v_00 DECIMAL(15,2);
    DECLARE v_01 DECIMAL(15,2);
    DECLARE v_02 DECIMAL(15,2);
    DECLARE v_03 DECIMAL(15,2);
    DECLARE v_04 DECIMAL(15,2);
    DECLARE v_05 DECIMAL(15,2);
    DECLARE v_06 DECIMAL(15,2);
    DECLARE v_07 DECIMAL(15,2);
    DECLARE v_08 DECIMAL(15,2);
    DECLARE v_09 DECIMAL(15,2);
    DECLARE v_10 DECIMAL(15,2);
    DECLARE v_11 DECIMAL(15,2);
    DECLARE v_12 DECIMAL(15,2);
    DECLARE v_13 DECIMAL(15,2);
    DECLARE v_14 DECIMAL(15,2);
    SELECT COL_001 INTO v_00
        FROM PNB.BRANCH_HIST_16_0091
        WHERE COL_000 = p_key;
    SET p_result = 0;
END;
