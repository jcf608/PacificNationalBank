-- ============================================================
-- DDL for TRANSACTION_HIST_15_0032
-- Pacific National Bank — DB2 Schema
-- ============================================================
CREATE TABLE PNB.TRANSACTION_HIST_15_0032 (
    COL_000    VARCHAR(60) ,
    COL_001    CHAR(1) NOT NULL,
    COL_002    DECIMAL(15,2) ,
    COL_003    CHAR(16) NOT NULL,
    COL_004    DECIMAL(9,6) NOT NULL,
    COL_005    BIGINT ,
    COL_006    VARCHAR(60) ,
    COL_007    DECIMAL(15,2) NOT NULL,
    COL_008    DECIMAL(9,6) ,
    COL_009    VARCHAR(200) ,
    COL_010    SMALLINT NOT NULL,
    COL_011    BIGINT NOT NULL,
    COL_012    CHAR(1) ,
    COL_013    CHAR(16) NOT NULL,
    COL_014    SMALLINT NOT NULL,
    COL_015    VARCHAR(60) ,
) IN PNBTS01;

CREATE UNIQUE INDEX PNB.IX_TRANSACTION_H150032_PK
    ON PNB.TRANSACTION_HIST_15_0032 (COL_000);

CREATE OR REPLACE PROCEDURE PNB.SP_TRANSACTION_H150032 (
    IN p_key CHAR(16),
    OUT p_result INTEGER
)
LANGUAGE SQL
BEGIN
    DECLARE v_00 DECIMAL(15,2);
    DECLARE v_01 DECIMAL(15,2);
    DECLARE v_02 DECIMAL(15,2);
    DECLARE v_03 DECIMAL(15,2);
    DECLARE v_04 DECIMAL(15,2);
    DECLARE v_05 DECIMAL(15,2);
    DECLARE v_06 DECIMAL(15,2);
    DECLARE v_07 DECIMAL(15,2);
    DECLARE v_08 DECIMAL(15,2);
    DECLARE v_09 DECIMAL(15,2);
    DECLARE v_10 DECIMAL(15,2);
    DECLARE v_11 DECIMAL(15,2);
    DECLARE v_12 DECIMAL(15,2);
    DECLARE v_13 DECIMAL(15,2);
    DECLARE v_14 DECIMAL(15,2);
    SELECT COL_001 INTO v_00
        FROM PNB.TRANSACTION_HIST_15_0032
        WHERE COL_000 = p_key;
    SET p_result = 0;
END;
