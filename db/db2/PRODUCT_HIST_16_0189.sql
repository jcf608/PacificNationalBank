-- ============================================================
-- DDL for PRODUCT_HIST_16_0189
-- Pacific National Bank — DB2 Schema
-- ============================================================
CREATE TABLE PNB.PRODUCT_HIST_16_0189 (
    COL_000    VARCHAR(60) ,
    COL_001    CHAR(16) NOT NULL,
    COL_002    CHAR(16) NOT NULL,
    COL_003    CHAR(3) ,
    COL_004    INTEGER ,
    COL_005    VARCHAR(60) ,
    COL_006    INTEGER ,
    COL_007    DATE ,
    COL_008    VARCHAR(200) NOT NULL,
    COL_009    CHAR(1) NOT NULL,
    COL_010    DECIMAL(15,2) NOT NULL,
    COL_011    TIMESTAMP ,
    COL_012    CHAR(3) NOT NULL,
    COL_013    SMALLINT NOT NULL,
    COL_014    VARCHAR(60) ,
    COL_015    SMALLINT NOT NULL,
    COL_016    VARCHAR(200) ,
) IN PNBTS01;

CREATE UNIQUE INDEX PNB.IX_PRODUCT_H160189_PK
    ON PNB.PRODUCT_HIST_16_0189 (COL_000);

CREATE OR REPLACE PROCEDURE PNB.SP_PRODUCT_H160189 (
    IN p_key CHAR(16),
    OUT p_result INTEGER
)
LANGUAGE SQL
BEGIN
    DECLARE v_00 DECIMAL(15,2);
    DECLARE v_01 DECIMAL(15,2);
    DECLARE v_02 DECIMAL(15,2);
    DECLARE v_03 DECIMAL(15,2);
    DECLARE v_04 DECIMAL(15,2);
    DECLARE v_05 DECIMAL(15,2);
    DECLARE v_06 DECIMAL(15,2);
    DECLARE v_07 DECIMAL(15,2);
    DECLARE v_08 DECIMAL(15,2);
    DECLARE v_09 DECIMAL(15,2);
    DECLARE v_10 DECIMAL(15,2);
    DECLARE v_11 DECIMAL(15,2);
    DECLARE v_12 DECIMAL(15,2);
    DECLARE v_13 DECIMAL(15,2);
    DECLARE v_14 DECIMAL(15,2);
    SELECT COL_001 INTO v_00
        FROM PNB.PRODUCT_HIST_16_0189
        WHERE COL_000 = p_key;
    SET p_result = 0;
END;
