-- ============================================================
-- DDL for PRODUCT_HIST_15_0188
-- Pacific National Bank — DB2 Schema
-- ============================================================
CREATE TABLE PNB.PRODUCT_HIST_15_0188 (
    COL_000    DATE NOT NULL,
    COL_001    VARCHAR(60) NOT NULL,
    COL_002    CHAR(1) NOT NULL,
    COL_003    VARCHAR(60) NOT NULL,
    COL_004    TIMESTAMP ,
    COL_005    CHAR(3) ,
    COL_006    DECIMAL(9,6) ,
    COL_007    VARCHAR(60) ,
    COL_008    VARCHAR(60) NOT NULL,
    COL_009    DECIMAL(9,6) ,
    COL_010    TIMESTAMP NOT NULL,
    COL_011    DECIMAL(15,2) NOT NULL,
    COL_012    CHAR(3) ,
    COL_013    CHAR(16) NOT NULL,
    COL_014    DECIMAL(9,6) ,
    COL_015    INTEGER NOT NULL,
    COL_016    CHAR(3) ,
    COL_017    DECIMAL(15,2) ,
    COL_018    DECIMAL(15,2) NOT NULL,
    COL_019    DECIMAL(15,2) ,
    COL_020    INTEGER ,
    COL_021    BIGINT NOT NULL,
    COL_022    VARCHAR(60) NOT NULL,
) IN PNBTS01;

CREATE UNIQUE INDEX PNB.IX_PRODUCT_H150188_PK
    ON PNB.PRODUCT_HIST_15_0188 (COL_000);

CREATE OR REPLACE PROCEDURE PNB.SP_PRODUCT_H150188 (
    IN p_key CHAR(16),
    OUT p_result INTEGER
)
LANGUAGE SQL
BEGIN
    DECLARE v_00 DECIMAL(15,2);
    DECLARE v_01 DECIMAL(15,2);
    DECLARE v_02 DECIMAL(15,2);
    DECLARE v_03 DECIMAL(15,2);
    DECLARE v_04 DECIMAL(15,2);
    DECLARE v_05 DECIMAL(15,2);
    DECLARE v_06 DECIMAL(15,2);
    DECLARE v_07 DECIMAL(15,2);
    DECLARE v_08 DECIMAL(15,2);
    DECLARE v_09 DECIMAL(15,2);
    DECLARE v_10 DECIMAL(15,2);
    DECLARE v_11 DECIMAL(15,2);
    DECLARE v_12 DECIMAL(15,2);
    DECLARE v_13 DECIMAL(15,2);
    DECLARE v_14 DECIMAL(15,2);
    SELECT COL_001 INTO v_00
        FROM PNB.PRODUCT_HIST_15_0188
        WHERE COL_000 = p_key;
    SET p_result = 0;
END;
