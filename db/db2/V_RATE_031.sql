...
CREATE VIEW PNB.V_RATE_031 AS
  SELECT
    ...
    COL_016,
    COL_017,
  FROM PNB.RATE_MASTER
  WHERE STATUS = 'A';

CREATE TRIGGER PNB.TRG_RATE_031_UPDATE
  AFTER UPDATE ON PNB.RATE_MASTER
  REFERENCING NEW AS N OLD AS O
  FOR EACH ROW
  MODE DB2SQL
  BEGIN ATOMIC
    INSERT INTO PNB.RATE_AUDIT (
...
CREATE OR REPLACE PROCEDURE PNB.SP_RATE_031 (
  IN  p_key       VARCHAR(32),
...
CREATE INDEX PNB.IX_RATE_031_00
  ON PNB.RATE_MASTER (COL_000 ASC);

CREATE INDEX PNB.IX_RATE_031_01
  ON PNB.RATE_MASTER (COL_001 ASC);

CREATE INDEX PNB.IX_RATE_031_02
  ON PNB.RATE_MASTER (COL_002 ASC);

CREATE INDEX PNB.IX_RATE_031_03
  ON PNB.RATE_MASTER (COL_003 ASC);

CREATE INDEX PNB.IX_RATE_031_04
  ON PNB.RATE_MASTER (COL_004 ASC);
