-- ============================================================
-- DDL for PRODUCT_HIST_16_0035
-- Pacific National Bank — DB2 Schema
-- ============================================================
CREATE TABLE PNB.PRODUCT_HIST_16_0035 (
    COL_000    SMALLINT ,
    COL_001    DECIMAL(15,2) ,
    COL_002    INTEGER ,
    COL_003    VARCHAR(200) NOT NULL,
    COL_004    BIGINT ,
    COL_005    SMALLINT NOT NULL,
    COL_006    CHAR(3) ,
    COL_007    DECIMAL(9,6) ,
    COL_008    CHAR(1) ,
    COL_009    CHAR(1) NOT NULL,
    COL_010    TIMESTAMP NOT NULL,
    COL_011    VARCHAR(60) NOT NULL,
    COL_012    VARCHAR(200) ,
    COL_013    DECIMAL(15,2) NOT NULL,
    COL_014    DECIMAL(9,6) ,
    COL_015    TIMESTAMP ,
    COL_016    VARCHAR(60) NOT NULL,
) IN PNBTS01;

CREATE UNIQUE INDEX PNB.IX_PRODUCT_H160035_PK
    ON PNB.PRODUCT_HIST_16_0035 (COL_000);

CREATE OR REPLACE PROCEDURE PNB.SP_PRODUCT_H160035 (
    IN p_key CHAR(16),
    OUT p_result INTEGER
)
LANGUAGE SQL
BEGIN
    DECLARE v_00 DECIMAL(15,2);
    DECLARE v_01 DECIMAL(15,2);
    DECLARE v_02 DECIMAL(15,2);
    DECLARE v_03 DECIMAL(15,2);
    DECLARE v_04 DECIMAL(15,2);
    DECLARE v_05 DECIMAL(15,2);
    DECLARE v_06 DECIMAL(15,2);
    DECLARE v_07 DECIMAL(15,2);
    DECLARE v_08 DECIMAL(15,2);
    DECLARE v_09 DECIMAL(15,2);
    DECLARE v_10 DECIMAL(15,2);
    DECLARE v_11 DECIMAL(15,2);
    DECLARE v_12 DECIMAL(15,2);
    DECLARE v_13 DECIMAL(15,2);
    DECLARE v_14 DECIMAL(15,2);
    SELECT COL_001 INTO v_00
        FROM PNB.PRODUCT_HIST_16_0035
        WHERE COL_000 = p_key;
    SET p_result = 0;
END;
