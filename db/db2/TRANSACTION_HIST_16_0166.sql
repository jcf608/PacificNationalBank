-- ============================================================
-- DDL for TRANSACTION_HIST_16_0166
-- Pacific National Bank — DB2 Schema
-- ============================================================
CREATE TABLE PNB.TRANSACTION_HIST_16_0166 (
    COL_000    DECIMAL(15,2) NOT NULL,
    COL_001    SMALLINT ,
    COL_002    CHAR(3) NOT NULL,
    COL_003    TIMESTAMP ,
    COL_004    BIGINT NOT NULL,
    COL_005    BIGINT NOT NULL,
    COL_006    DATE ,
    COL_007    INTEGER ,
    COL_008    CHAR(16) ,
    COL_009    DECIMAL(15,2) ,
    COL_010    CHAR(16) NOT NULL,
    COL_011    CHAR(3) ,
    COL_012    BIGINT ,
    COL_013    DECIMAL(15,2) ,
    COL_014    VARCHAR(200) NOT NULL,
    COL_015    DECIMAL(9,6) ,
    COL_016    CHAR(1) ,
) IN PNBTS01;

CREATE UNIQUE INDEX PNB.IX_TRANSACTION_H160166_PK
    ON PNB.TRANSACTION_HIST_16_0166 (COL_000);

CREATE OR REPLACE PROCEDURE PNB.SP_TRANSACTION_H160166 (
    IN p_key CHAR(16),
    OUT p_result INTEGER
)
LANGUAGE SQL
BEGIN
    DECLARE v_00 DECIMAL(15,2);
    DECLARE v_01 DECIMAL(15,2);
    DECLARE v_02 DECIMAL(15,2);
    DECLARE v_03 DECIMAL(15,2);
    DECLARE v_04 DECIMAL(15,2);
    DECLARE v_05 DECIMAL(15,2);
    DECLARE v_06 DECIMAL(15,2);
    DECLARE v_07 DECIMAL(15,2);
    DECLARE v_08 DECIMAL(15,2);
    DECLARE v_09 DECIMAL(15,2);
    DECLARE v_10 DECIMAL(15,2);
    DECLARE v_11 DECIMAL(15,2);
    DECLARE v_12 DECIMAL(15,2);
    DECLARE v_13 DECIMAL(15,2);
    DECLARE v_14 DECIMAL(15,2);
    SELECT COL_001 INTO v_00
        FROM PNB.TRANSACTION_HIST_16_0166
        WHERE COL_000 = p_key;
    SET p_result = 0;
END;
