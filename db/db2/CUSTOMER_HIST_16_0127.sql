-- ============================================================
-- DDL for CUSTOMER_HIST_16_0127
-- Pacific National Bank — DB2 Schema
-- ============================================================
CREATE TABLE PNB.CUSTOMER_HIST_16_0127 (
    COL_000    INTEGER NOT NULL,
    COL_001    CHAR(16) ,
    COL_002    DATE NOT NULL,
    COL_003    BIGINT ,
    COL_004    SMALLINT NOT NULL,
    COL_005    CHAR(3) NOT NULL,
    COL_006    CHAR(3) ,
    COL_007    BIGINT ,
    COL_008    CHAR(3) ,
    COL_009    VARCHAR(60) NOT NULL,
    COL_010    VARCHAR(60) ,
    COL_011    VARCHAR(60) NOT NULL,
    COL_012    DATE NOT NULL,
    COL_013    DECIMAL(15,2) ,
    COL_014    DECIMAL(9,6) NOT NULL,
    COL_015    DECIMAL(9,6) NOT NULL,
) IN PNBTS01;

CREATE UNIQUE INDEX PNB.IX_CUSTOMER_H160127_PK
    ON PNB.CUSTOMER_HIST_16_0127 (COL_000);

CREATE OR REPLACE PROCEDURE PNB.SP_CUSTOMER_H160127 (
    IN p_key CHAR(16),
    OUT p_result INTEGER
)
LANGUAGE SQL
BEGIN
    DECLARE v_00 DECIMAL(15,2);
    DECLARE v_01 DECIMAL(15,2);
    DECLARE v_02 DECIMAL(15,2);
    DECLARE v_03 DECIMAL(15,2);
    DECLARE v_04 DECIMAL(15,2);
    DECLARE v_05 DECIMAL(15,2);
    DECLARE v_06 DECIMAL(15,2);
    DECLARE v_07 DECIMAL(15,2);
    DECLARE v_08 DECIMAL(15,2);
    DECLARE v_09 DECIMAL(15,2);
    DECLARE v_10 DECIMAL(15,2);
    DECLARE v_11 DECIMAL(15,2);
    DECLARE v_12 DECIMAL(15,2);
    DECLARE v_13 DECIMAL(15,2);
    DECLARE v_14 DECIMAL(15,2);
    SELECT COL_001 INTO v_00
        FROM PNB.CUSTOMER_HIST_16_0127
        WHERE COL_000 = p_key;
    SET p_result = 0;
END;
