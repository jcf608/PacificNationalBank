-- ============================================================
-- DDL for ACCOUNT_HIST_15_0136
-- Pacific National Bank — DB2 Schema
-- ============================================================
CREATE TABLE PNB.ACCOUNT_HIST_15_0136 (
    COL_000    CHAR(1) ,
    COL_001    VARCHAR(60) NOT NULL,
    COL_002    SMALLINT ,
    COL_003    DECIMAL(9,6) ,
    COL_004    DECIMAL(9,6) ,
    COL_005    TIMESTAMP ,
    COL_006    CHAR(3) NOT NULL,
    COL_007    DECIMAL(15,2) ,
    COL_008    CHAR(3) ,
    COL_009    DECIMAL(15,2) ,
    COL_010    VARCHAR(60) ,
    COL_011    SMALLINT NOT NULL,
    COL_012    SMALLINT ,
    COL_013    CHAR(1) NOT NULL,
    COL_014    VARCHAR(200) ,
    COL_015    BIGINT ,
    COL_016    VARCHAR(200) NOT NULL,
    COL_017    DECIMAL(15,2) NOT NULL,
) IN PNBTS01;

CREATE UNIQUE INDEX PNB.IX_ACCOUNT_H150136_PK
    ON PNB.ACCOUNT_HIST_15_0136 (COL_000);

CREATE OR REPLACE PROCEDURE PNB.SP_ACCOUNT_H150136 (
    IN p_key CHAR(16),
    OUT p_result INTEGER
)
LANGUAGE SQL
BEGIN
    DECLARE v_00 DECIMAL(15,2);
    DECLARE v_01 DECIMAL(15,2);
    DECLARE v_02 DECIMAL(15,2);
    DECLARE v_03 DECIMAL(15,2);
    DECLARE v_04 DECIMAL(15,2);
    DECLARE v_05 DECIMAL(15,2);
    DECLARE v_06 DECIMAL(15,2);
    DECLARE v_07 DECIMAL(15,2);
    DECLARE v_08 DECIMAL(15,2);
    DECLARE v_09 DECIMAL(15,2);
    DECLARE v_10 DECIMAL(15,2);
    DECLARE v_11 DECIMAL(15,2);
    DECLARE v_12 DECIMAL(15,2);
    DECLARE v_13 DECIMAL(15,2);
    DECLARE v_14 DECIMAL(15,2);
    SELECT COL_001 INTO v_00
        FROM PNB.ACCOUNT_HIST_15_0136
        WHERE COL_000 = p_key;
    SET p_result = 0;
END;
