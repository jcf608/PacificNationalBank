-- ============================================================
-- DDL for PRODUCT_HIST_16_0066
-- Pacific National Bank — DB2 Schema
-- ============================================================
CREATE TABLE PNB.PRODUCT_HIST_16_0066 (
    COL_000    CHAR(1) NOT NULL,
    COL_001    INTEGER ,
    COL_002    VARCHAR(60) ,
    COL_003    CHAR(16) NOT NULL,
    COL_004    CHAR(16) ,
    COL_005    CHAR(1) ,
    COL_006    DATE NOT NULL,
    COL_007    BIGINT NOT NULL,
    COL_008    VARCHAR(200) NOT NULL,
    COL_009    VARCHAR(200) ,
    COL_010    CHAR(1) NOT NULL,
    COL_011    CHAR(16) ,
    COL_012    VARCHAR(200) ,
    COL_013    DECIMAL(15,2) ,
    COL_014    TIMESTAMP NOT NULL,
    COL_015    BIGINT ,
    COL_016    DECIMAL(15,2) ,
) IN PNBTS01;

CREATE UNIQUE INDEX PNB.IX_PRODUCT_H160066_PK
    ON PNB.PRODUCT_HIST_16_0066 (COL_000);

CREATE OR REPLACE PROCEDURE PNB.SP_PRODUCT_H160066 (
    IN p_key CHAR(16),
    OUT p_result INTEGER
)
LANGUAGE SQL
BEGIN
    DECLARE v_00 DECIMAL(15,2);
    DECLARE v_01 DECIMAL(15,2);
    DECLARE v_02 DECIMAL(15,2);
    DECLARE v_03 DECIMAL(15,2);
    DECLARE v_04 DECIMAL(15,2);
    DECLARE v_05 DECIMAL(15,2);
    DECLARE v_06 DECIMAL(15,2);
    DECLARE v_07 DECIMAL(15,2);
    DECLARE v_08 DECIMAL(15,2);
    DECLARE v_09 DECIMAL(15,2);
    DECLARE v_10 DECIMAL(15,2);
    DECLARE v_11 DECIMAL(15,2);
    DECLARE v_12 DECIMAL(15,2);
    DECLARE v_13 DECIMAL(15,2);
    DECLARE v_14 DECIMAL(15,2);
    SELECT COL_001 INTO v_00
        FROM PNB.PRODUCT_HIST_16_0066
        WHERE COL_000 = p_key;
    SET p_result = 0;
END;
