-- ============================================================
-- DDL for PRODUCT_HIST_16_0150
-- Pacific National Bank — DB2 Schema
-- ============================================================
CREATE TABLE PNB.PRODUCT_HIST_16_0150 (
    COL_000    BIGINT NOT NULL,
    COL_001    CHAR(3) ,
    COL_002    CHAR(1) NOT NULL,
    COL_003    TIMESTAMP NOT NULL,
    COL_004    VARCHAR(60) ,
    COL_005    SMALLINT ,
    COL_006    DECIMAL(15,2) ,
    COL_007    INTEGER NOT NULL,
    COL_008    CHAR(3) NOT NULL,
    COL_009    VARCHAR(60) ,
    COL_010    SMALLINT ,
    COL_011    BIGINT ,
    COL_012    INTEGER ,
    COL_013    CHAR(16) NOT NULL,
    COL_014    DATE NOT NULL,
    COL_015    BIGINT NOT NULL,
    COL_016    TIMESTAMP ,
) IN PNBTS01;

CREATE UNIQUE INDEX PNB.IX_PRODUCT_H160150_PK
    ON PNB.PRODUCT_HIST_16_0150 (COL_000);

CREATE OR REPLACE PROCEDURE PNB.SP_PRODUCT_H160150 (
    IN p_key CHAR(16),
    OUT p_result INTEGER
)
LANGUAGE SQL
BEGIN
    DECLARE v_00 DECIMAL(15,2);
    DECLARE v_01 DECIMAL(15,2);
    DECLARE v_02 DECIMAL(15,2);
    DECLARE v_03 DECIMAL(15,2);
    DECLARE v_04 DECIMAL(15,2);
    DECLARE v_05 DECIMAL(15,2);
    DECLARE v_06 DECIMAL(15,2);
    DECLARE v_07 DECIMAL(15,2);
    DECLARE v_08 DECIMAL(15,2);
    DECLARE v_09 DECIMAL(15,2);
    DECLARE v_10 DECIMAL(15,2);
    DECLARE v_11 DECIMAL(15,2);
    DECLARE v_12 DECIMAL(15,2);
    DECLARE v_13 DECIMAL(15,2);
    DECLARE v_14 DECIMAL(15,2);
    SELECT COL_001 INTO v_00
        FROM PNB.PRODUCT_HIST_16_0150
        WHERE COL_000 = p_key;
    SET p_result = 0;
END;
