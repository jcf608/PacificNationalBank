-- ============================================================
-- DDL for BRANCH_HIST_15_0197
-- Pacific National Bank — DB2 Schema
-- ============================================================
CREATE TABLE PNB.BRANCH_HIST_15_0197 (
    COL_000    DECIMAL(15,2) NOT NULL,
    COL_001    TIMESTAMP ,
    COL_002    CHAR(3) NOT NULL,
    COL_003    DATE ,
    COL_004    SMALLINT NOT NULL,
    COL_005    VARCHAR(200) ,
    COL_006    SMALLINT ,
    COL_007    DECIMAL(9,6) NOT NULL,
    COL_008    DECIMAL(9,6) ,
    COL_009    DECIMAL(15,2) ,
    COL_010    DECIMAL(9,6) ,
    COL_011    DECIMAL(9,6) ,
    COL_012    VARCHAR(60) NOT NULL,
    COL_013    CHAR(16) NOT NULL,
    COL_014    INTEGER NOT NULL,
    COL_015    VARCHAR(200) NOT NULL,
    COL_016    CHAR(3) ,
) IN PNBTS01;

CREATE UNIQUE INDEX PNB.IX_BRANCH_H150197_PK
    ON PNB.BRANCH_HIST_15_0197 (COL_000);

CREATE OR REPLACE PROCEDURE PNB.SP_BRANCH_H150197 (
    IN p_key CHAR(16),
    OUT p_result INTEGER
)
LANGUAGE SQL
BEGIN
    DECLARE v_00 DECIMAL(15,2);
    DECLARE v_01 DECIMAL(15,2);
    DECLARE v_02 DECIMAL(15,2);
    DECLARE v_03 DECIMAL(15,2);
    DECLARE v_04 DECIMAL(15,2);
    DECLARE v_05 DECIMAL(15,2);
    DECLARE v_06 DECIMAL(15,2);
    DECLARE v_07 DECIMAL(15,2);
    DECLARE v_08 DECIMAL(15,2);
    DECLARE v_09 DECIMAL(15,2);
    DECLARE v_10 DECIMAL(15,2);
    DECLARE v_11 DECIMAL(15,2);
    DECLARE v_12 DECIMAL(15,2);
    DECLARE v_13 DECIMAL(15,2);
    DECLARE v_14 DECIMAL(15,2);
    SELECT COL_001 INTO v_00
        FROM PNB.BRANCH_HIST_15_0197
        WHERE COL_000 = p_key;
    SET p_result = 0;
END;
