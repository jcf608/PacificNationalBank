...
CREATE VIEW PNB.V_CREDITLINE_032 AS
  SELECT
    COL_000,
    COL_001,
    COL_002,
    ...
    COL_013,
    COL_014,
  FROM PNB.CREDITLINE_MASTER
  WHERE STATUS = 'A';

CREATE TRIGGER PNB.TRG_CREDITLINE_032_UPDATE
  AFTER UPDATE ON PNB.CREDITLINE_MASTER
  REFERENCING NEW AS N OLD AS O
  FOR EACH ROW
  MODE DB2SQL
  BEGIN ATOMIC
    INSERT INTO PNB.CREDITLINE_AUDIT (
...
CREATE OR REPLACE PROCEDURE PNB.SP_CREDITLINE_032 (
  IN  p_key       VARCHAR(32),
...
CREATE INDEX PNB.IX_CREDITLINE_032_00
  ON PNB.CREDITLINE_MASTER (COL_000 ASC);

CREATE INDEX PNB.IX_CREDITLINE_032_01
  ON PNB.CREDITLINE_MASTER (COL_001 ASC);
